...
export default function FocusAnalytics() {
  ...
  const loadData = async () => {
    setStats({
      ...
      interruptionCount: 8,
      focusScore: 72,
      productiveHours: { '09': 60, '10': 75, '11': 45, '14': 50, '15': 40, '16': 30 },
    });

    setSessions([
      { id: 1, start_time: new Date(Date.now() - 3600000).toISOString(), end_time: new Date().toISOString(), duration_minutes: 60, type: 'deep_work', score: 85, interruptions: 2 },
      { id: 2, start_time: new Date(Date.now() - 7200000).toISOString(), end_time: new Date(Date.now() - 3600000).toISOString(), duration_minutes: 45, type: 'shallow_work', score: 65, interruptions: 4 },
      { id: 3, start_time: new Date(Date.now() - 10800000).toISOString(), end_time: new Date(Date.now() - 7200000).toISOString(), duration_minutes: 30, type: 'meeting', score: 70, interruptions: 1 },
    ]);
  };
    ...
    <>
      ...
      <div className="page-body">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '20px' }}>
          {/* Left: Productivity Chart & Sessions */}
          <div>
            {/* Productive Hours */}
            <div className="card" style={{ padding: '20px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', height: 120 }}>
                {Array.from({ length: 24 }, (_, i) => {
                  ...
                  const height = (value / maxProductivity) * 100;
                  return (
                ...
                {[
                  ...
                  <div key={item.label}>
                    ...
                    <div style={{ height: 4, background: 'var(--color-bg)', borderRadius: 2, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${(item.value / stats.totalFocusMinutes) * 100}%`, background: item.color, borderRadius: 2 }} />
                    </div>
                  </div>
                ))}
              </div>
